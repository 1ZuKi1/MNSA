...
Verified with 81 unit tests (permissions, approval chain, dates, document types, co-departments, record fields, session cookie, jobs, the members import) and a 228-step end-to-end test driving every role through the real server, plus a production-build check with `wrangler dev` and an axe accessibility audit of every public page.
