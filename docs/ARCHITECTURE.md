...
**A second department** («Хамтран хариуцах хэлтэс», optional on every document, 2026-09-24): the document shows in both departments' archives and lists, both read it as their own, and the second department's дарга approves it too — a `cohead` step right after the main дарга, or first for a type without one (`chainFor` in `workflow.ts`). Numbering and editing stay with the main department; the second дарга signs beside the main дарга wherever the main дарга signs, and is always in the printed approval line. Leadership as the second department adds no step (it has no дарга). Stored as `records.co_department_id` (migration 0006).
...
| 5c · «Ажлууд» — jobs outside events, with a stage board | ✅ (2026-09-26) |
| 5 · Live meeting minutes | not started |
| 6 · Presidency handover page, weekly backup | not started |

Verified with 80 unit tests (permissions, approval chain, dates, document types, co-departments, record fields, session cookie, jobs, the members import) and a 218-step end-to-end test driving every role through the real server, plus a production-build check with `wrangler dev` and an axe accessibility audit of every public page.
...
- **«Ажлууд» — work that isn't an event (2026-09-26).** Staff asked for a place for the rest of the work: preparing a budget, booking a hall, updating a list. A job belongs to one department and has one accountable person (хариуцагч) who moves it through three stages — Эхлээгүй → Хийгдэж байна → Дууссан — with optional progress notes; the whole history shows on the job's page. «Зөвхөн манай хэлтэс» jobs are seen by that department, the хариуцагч and the leadership; «Бүх гишүүд» jobs by everyone in the workspace. Never on the public site. Anyone may add a job to their own department (the leadership anywhere); whoever set it up, that department's дарга and the leadership may change or cancel it; the хариуцагч may move it along. `/ajil` is a three-column board (done jobs stay 30 days); the dashboard lists your open jobs with a one-tap «Эхлүүлэх»/«Дууссан», the menu counts them, the department page lists its open jobs, and a newly assigned хариуцагч gets an e-mail. Tables `jobs` and `job_updates` (migration 0007).
- **Fixes after the first staff demo (2026-09-26).** (1) *Events took 5 minutes to appear* — the home and events pages were edge-cached for 5 minutes; no database page is cached any more (only photos, which never change). (2) *Tapping a document opened the one below it, or nothing* — whole-row links were a link stretched over the table row, which some browsers (Safari; and anything with its own layer, like the status badge) place wrongly; a row now opens by a small click handler, and the title stays a real link for keyboards. (3) *Back didn't go back* — every save posted and came back to the same page, adding a second copy of it to the history. Staff forms now post in the background and the result replaces the page in the history (`location.replace`); a page that comes back with a form error is shown in place, with what was typed. Without JavaScript, forms post as before.
- **Team passphrase while codes are on screen (2026-09-25).** Running the real team on `workers.dev` meant anyone who knew a member's e-mail (easy to guess: student ID @stu.pku.edu.cn) could log in as them, President included. Until e-mail sending works, the login page on `*.workers.dev` also asks for a shared passphrase (secret `STAFF_GATE` on `mnsa-dep`), checked after the rate limit so guessing costs the same as asking for codes. It keeps outsiders out; it can't stop one member logging in as another, which only real e-mail delivery fixes — so the domain and Resend stay the priority.
- **Roles and departments always fit (2026-09-25).** Heads and members belong to one of the five departments; the President and the board to the leadership; the maintainer to none. Invites and role changes that break this are refused (`placeRole`), and the pickers on the page disable the wrong options.
- **Photos on the team page (2026-09-24).** The President and the board share the first row; below, each department is a card as wide as its people need, so cards share rows (on a laptop the whole team fits in about two screens; on a phone each department is one card with its people side by side). Each person is a round photo with the name and role under it, the head first. No photo → the person's initials on their department's colour. The photo is uploaded on the person's page in *Гишүүд* by the person themselves, by whoever may change their account, or by the maintainer (who loads the team's photos each September — he can open every member page for that, but changes nothing else there). The browser shrinks it to ≤900 px before upload; the whole image is kept and the circle crop happens in CSS (`object-position: 50% 25%`, so heads aren't cut off). Replacing or removing a photo frees the old bytes; both are in the audit log. `users.photo_id` → a row in `mnsa-media`.
...
- **The full Үндсэн дүрэм is public** as a PDF (`public/files/undsen-durem-2025-11-08.pdf`, the Анхдугаар шинэчилсэн найруулга, 27 pages): a section on *Танилцуулга* with «Унших» and «Татаж авах» and the 13 chapters, and a link in every footer. Every member has the right to all of the association's information (10.1.4, 52). Served straight from Cloudflare's static assets (`/files/*` bypasses the Worker), so it costs nothing per download. Versions are dated in the file name and kept.
