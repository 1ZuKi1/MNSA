...
Verified with 74 unit tests (permissions, approval chain, dates, document types, co-departments, record fields, session cookie, the members import) and a 193-step end-to-end test driving every role through the real server, plus a production-build check with `wrangler dev` and an axe accessibility audit of every public page.
